...
input= ['output files/filtered_proteins_cleavable_mts.fasta', 'output files/filtered_proteins_no_cleavable_mts.fasta', 'output files/filtered_proteins.fasta']

#wanted_result either "absolute" or "hgt"
wanted_result = "absulute"
        ...
        for i in range(rows):
            for j in range(cols):
                if amino_acid[j] in counted_instances[i]:
                    ...
                    p_value = hypergeom.cdf(x, M, n, N)
                    abs_log_val = abs(np.log10(p_value))
                    f_obs = x / N
                    f_exp = n / M
                    print(f_obs, f_exp)
                    if f_obs < f_exp:
                        abs_log_val = -1 * abs_log_val
                    else:
                        abs_log_val = abs_log_val
                    visual_array[j, i] = abs_log_val
...
pcm= ax[0].imshow(all_arrays[0], cmap="Blues")
pcm = ax[1].imshow(all_arrays[1], cmap="Blues")
