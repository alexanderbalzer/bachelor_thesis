...
def classify_location(locations):
    """
    Classify the location based on fuzzy keyword matching.
    Args:
        locations (list): List of location strings.
    Returns:
        str: Classified location.
    """
...
def run(general_working_dir, organism_names, batch_size=500):
    """
    Main function to run the pipeline. Takes a list of organisms, loads their fasta and GO annotation files,
    gets the GO terms for each protein, classifies the location of each protein, and writes the results to a new CSV file.
    Args:
